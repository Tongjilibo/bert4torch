...
class LoraCallback(Callback):
    '''基于loralib的实现
    '''
    def __init__(self, lora_rank, lora_alpha=1, lora_train_bias='none', **kwargs):
    ...
    def on_train_begin(self, logs=None):
        from .layers import convert_to_lora_recursively
        import loralib as lora
        if self.lora_rank > 0:
            params_before = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            convert_to_lora_recursively(self.model, self.lora_rank, self.lora_alpha)
            lora.mark_only_lora_as_trainable(self.model, self.lora_train_bias)
            params_after = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
            print(f"Using lora training and reduce trainable parameters from {params_before} to {params_after} ({params_after/params_before:.2f}%)")
        else:
            print("[WARNING] To use lora training, Args `lora_rank` should > 0")