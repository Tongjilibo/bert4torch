...
class InternVL(PreTrainedModelForDecoder):
    ...
    def __init__(self, **config):
        ...
        if self.config.llm_config.architectures == 'Llama':
            self.language_model = LLaMA(self.config.llm_config)
        elif self.config.llm_config.architectures == 'Qwen2':
    ...
    def get_vllm_embedding(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                pixel_values_videos = pixel_values_videos.type(self.vision_model.get_dtype())
    ...
    def forward(self, *inputs:Union[tuple, list], **model_kwargs):
        ...
        input_ids, _, _, _, _, model_kwargs['attention_mask'], model_kwargs = self.language_model.preprare_embeddings_inputs(*inputs, **model_kwargs)
