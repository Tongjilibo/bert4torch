...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        out_chans: int = 256,
        qkv_bias: bool = True,
        downsample_channels: List[int] = None,
        ...
        self.net_2 = nn.Conv2d(256, downsample_channels[0], kernel_size=3, stride=2, padding=1, bias=False)
        self.net_3 = nn.Conv2d(downsample_channels[0], downsample_channels[1], kernel_size=3, stride=2, padding=1, bias=False)
...
def build_sam_fast_vit_b(compile_mode='max-autotune', dtype=torch.bfloat16, **config):
    image_encoder = build_sam_vit_b(**config).eval().to(dtype)
