...
class AutoFeatureExtractor:
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        has_local_code = feature_extractor_class is not None or type(config).__name__ in FEATURE_EXTRACTOR_MAPPING
        if has_remote_code:
        ...
        elif type(config).__name__ in FEATURE_EXTRACTOR_MAPPING:
            feature_extractor_class = FEATURE_EXTRACTOR_MAPPING[type(config).__name__]
            return feature_extractor_class.from_dict(config_dict, **kwargs)

        raise ValueError(
            f"Unrecognized feature extractor in {pretrained_model_name_or_path}. Should have a "
            f"`feature_extractor_type` key in its {FEATURE_EXTRACTOR_NAME} of {CONFIG_NAME}, or one of the following "
            f"`model_type` keys in its {CONFIG_NAME}: {', '.join(c for c in FEATURE_EXTRACTOR_MAPPING)}"
        )
