...
# 模型路径
model_dir = '/data/pretrain_ckpt/TsinghuaAI/CPM-Generate'
...
class TextExpansion(AutoRegressiveDecoder):
    ...
    def generate(self, text, n=1, top_p=0.95, temperature=1):
        """输出结果会有一定的随机性，如果只关心Few Shot效果，
        可以考虑将解码方式换为beam search。
        """
        token_ids, _ = tokenizer.encode(text)
        results = self.random_sample([token_ids], n=n, top_p=top_p, temperature=temperature)  # 基于随机采样
