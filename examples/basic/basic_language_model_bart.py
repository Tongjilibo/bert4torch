...
encoder = BartForConditionalGeneration.from_pretrained("F:/Projects/pretrain_ckpt/bart/[FudanNLP_torch_base]/")

input_ids = tokenizer.encode("北京是[MASK]的首都", return_tensors='pt')
pred_ids = encoder.generate(input_ids, num_beams=4, max_length=20)
...
encoder = build_transformer_model(config_path, checkpoint_path, model='bart', segment_vocab_size=0).to(device)
