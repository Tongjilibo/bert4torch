...
class UIEPredictor(object):
    ...
    def _single_stage_predict(self, inputs):
        ...
        token_ids, segment_ids, offset = self._tokenizer.encode(short_texts_prompts, short_input_texts, maxlen=self._max_seq_len, return_offsets=True)
        offset_maps = []
        for j in offset:
            tmp, minus = [], 0
            for i, k in enumerate(j):
                if not k:
                    ...
                    tmp.append([k[0], i-minus])
            offset_maps.append(tmp)
